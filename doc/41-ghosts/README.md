# 41 · 影 / 鬼谱条目

> 每只影一个 md 文件。规则与模板见 [`../40-ghost-rules.md`](../40-ghost-rules.md)。
> 加新影走 `/new-ghost` 斜杠命令，会自动建文件 + 更新本索引。
>
> **2026-05-01 v2 收紧**：v1 的 8 张旧鬼卡（校园 + 民俗）全部归档。本目录只维护 v2 的 6 张新影卡。

## 命名约定

- **prose / UI 里**：写作"影 / 余像 / 旧时人 / 山的记忆"
- **技术 ID 仍叫 `ghost-*`**：保留 v1 的 ID 前缀避免破坏所有引用

## v2 当前条目（6 只）

### Boss 影（2 只）

| ID | 显示名 | 段位 | 教学概念 | 状态 |
|---|---|---|---|---|
| `ghost-feng-kou-shan-jun` | 风口山君 | 段 4 风口 | 综合：快门 × ISO 噪点 | ⚠️ 待建 |
| `ghost-shan-ding-shou-zhe` | 山顶守者 | 段 6 顶峰 | 综合：景深 × 对焦 × 长曝 | ⚠️ 待建 |

### 扰乱影（4 只）

| ID | 显示名 | 段位 | 教学概念 | 状态 |
|---|---|---|---|---|
| `ghost-lin-ci-jing-zuo` | 林祠静坐者 | 段 1 林线 | 半按对焦 + 中央构图 | ⚠️ 待建 |
| `ghost-wu-li-can-ying` | 雾里残影 | 段 2 雾带 | 长曝消形 | ⚠️ 待建 |
| `ghost-shan-ya-lue-ying` | 山鸦掠影 | 段 3 岩壁 | 快门冻结 | ⚠️ 待建 |
| `ghost-xing-ling` | 星灵 | 段 5 星脊 | 长曝积累 / 星轨 | ⚠️ 待建 |

> ⚠️ 待建 = ID 已占位，影卡 md 还没建。跑 `/new-ghost` 落卡后状态转 🟡 In-Progress。
> ⚠️ 用 `/new-ghost` 时**口头明确告知 v2 治愈基调 + 山系**，否则 agent 可能按 v1 民俗调性写。

## v1 鬼卡已物理删除

v1 设计阶段的 8 张鬼卡（ghost-yundong-yuan / ghost-guniang / ghost-yingzi / ghost-xianying / ghost-beimian / ghost-fenshen / ghost-xiaomian / ghost-qintong）已从仓库彻底删除——历史在 git log 里查。详见 [`../plan/rules-revisions.md` 2026-05-01 v2 cutover](../plan/rules-revisions.md)。
