---
name: camera-tuner
description: 把游戏设计意图翻译成具体的相机 / 鬼 / 评分数值。例如"我希望这只鬼需要 1/500 以上快门才能拍清"→ 反算鬼的移动速度、blur_multiplier、passingScore、该关卡的 ISO/光圈解锁限制等。也可反向：给一组数值问"玩家会被迫做出什么选择"。用在做鬼 / 做关卡 / 调数值卡脖子时。
...
| 方向 | 典型提问 | 你要算什么 |
|---|---|---|
| 正向：玩法 → 数值 | "让这只鬼需要快门 ≥ 1/500 才能拍清" | 鬼的移动速度、blur_multiplier、passingScore、关卡亮度 |
| 反向：数值 → 玩法 | "如果 ISO 给到 3200，玩家会怎么玩" | 在什么亮度下玩家会被迫抬 ISO，抬到哪里开始被噪点惩罚 |
| 对齐检查 | "这个关卡的参数组合，能不能拍到这只鬼" | 扫一遍 Boss 条件和关卡亮度/阶段，指出矛盾 |
...
3. **一个自洽性警告清单**（例如："你这组数值要求玩家开到 ISO 3200，但当前章节只解锁到 ISO 1600。要么调高章节门槛，要么把这只鬼放后面的章节。"）
